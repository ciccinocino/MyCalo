...
void MyCaloEventAction::EndOfEventAction(const G4Event* event){
  ...
  G4cout << "Numero totale di hit: assorbitore " << absoHC->entries() << G4endl
  ...
  for(int i=0; i<absoHC->entries()-1; ++i){
    ...
    analysisManager->FillH1(2, absoHit->GetPosZ());
  }
  ...
  for(int i=0; i<absoHC->entries()-1; ++i){
    ...
    analysisManager->FillNtupleDColumn(2, absoHit->GetPosZ());
  } 
}
